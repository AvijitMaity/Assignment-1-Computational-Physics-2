'''
This program shows how The Gauss Seidel method is easily derived by examining each of the
...
while np.any(abs(x-xt)>0.01) :  # tolerance of 0.01
    i+=1
    #In matrix terms, the definition of the Gauss Seidel method in can be expressed as  x = Tx+c
    x = np.dot(T,x)+C
